...
function App() {
  ...
  const changeTestobotX = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (
      testobotWouldNotCrash(parseInt(event.target.value), testobotY) &&
  ...
  const changeTestobotY = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (
      testobotWouldNotCrash(testobotX, parseInt(event.target.value)) &&
      isNaN(parseInt(event.target.value)) === false
    ) {
    ...
    switch (testobotDirection) {
      case "east":
        if (clockwise) {
          ...
          setTestobotDirection("north");
        }
        break;
      case "south":
        if (clockwise) {
          setTestobotDirection("west");
        } else {
          setTestobotDirection("east");
        }
        break;
      case "west":
        if (clockwise) {
  ...
  return (
    <div className="app">
      ...
      <div>
        <div>
          <Settings
            changeRoomWidth={changeRoomWidth}
            changeRoomHeight={changeRoomHeight}
            changeTestobotX={changeTestobotX}
            changeTestobotY={changeTestobotY}
            testobotDirection={testobotDirection}
            roomWidth={roomWidth}
          ...
          <RemoteControl
            turnLeft={changeTestobotDirection}
            turnRight={changeTestobotDirection}
            moveForward={moveTestobotForward}
          ></RemoteControl>
        </div>
        <Room
          roomWidth={roomWidth}
